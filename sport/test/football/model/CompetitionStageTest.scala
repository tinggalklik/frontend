  ...
  "stagesFromCompetition" - {
    ...
    "for groups" - {
      ...
      "if there are multiple stages" - {
        ...
        lazy val leagueTableEntries0 = stages(0).asInstanceOf[Groups].groupTables.map(_._2).flatten.toSet
        lazy val leagueTableEntries1 = stages(1).asInstanceOf[Groups].groupTables.map(_._2).flatten.toSet
